...
{
    $a = floatval($_POST["a"]); // Ép kiểu biến a thành số thập phân
    $b = floatval($_POST["b"]); // Ép kiểu biến b thành số thập phân
    $x = 0;
    if ($a == 0)
    {
        if ($b == 0)
    ...
    {
        $x = -$b / $a;
        $x = round($x, 2); // Làm tròn x đến 2 chữ số thập phân
        $nghiem = "x = $x";
    }
}
?>
<form action="cau7.php" method="post">
